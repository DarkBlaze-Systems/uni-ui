...
fn build_ui() -> (Document, NodeId, NodeId, NodeId) {
    ...
    doc.apply_from(
        ...
        Mutation::SetProp {
            id: headline,
            key: "content".into(),
            value: Value::Text("Welcome back".into()),
        },
    )
    .unwrap();
    doc.apply_from(
...
fn main() {
    ...
    println!(
        "Document built: {} nodes, {} audited edits so far.\n",
        doc.len(),
        doc.audit_log().len()
    );
    ...
    println!(
        "User profile: text_scale={}, high_contrast={}, density={}",
        profile.text_scale, profile.high_contrast, profile.density
    );
    println!(
        "Env: {}x{}, width_class={:?}, touch={}\n",
        env.win_w,
        env.win_h,
        env.width_class(),
        env.is_touch()
